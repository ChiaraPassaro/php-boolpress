...
var url = window.location.origin + '/php-boolpress/comments.php'; //creo obj search per conservare dati chiamata ajax
...
$(document).ready(function () {
  var getData = window.location.search.substring(1).split('='); //chiamata ajax per visualizzare commenti
  ...
  $searchInput.keyup(function (event) {
    var text; //se premo un tasto qualunque che non sia enter, delete and backspace

    if (event.keyCode !== 46 && event.keyCode !== 8 && event.keyCode !== 13) {
      //prendo il valore dell'input
      text = $(this).val(); //se search.text è undefined
      ...
      if (search.text === undefined) {
        if (search.pending) {
          ...
          search.text = text; //setto su pending

          search.pending = true; //chiamo funzione getTags per fae chiamata ajax e prendere tutti i tag che iniziano con quella prima lettera
        ...
        var lastFilteredTags = searchTag(search.tags, text); //stampo i risultati filtrandoli ulteriormente
      ...
      text = $(this).val(); //Se l'input è vuoto

      if (text.length === 0) {
        //cancello il valore di search.text
        search.text = undefined; //richiamo printSuggestions per cancellare i suggerimenti

        printSuggestions();
      } else {
        //altrimenti setto il nuovo valore di search.text
        search.text = text; //richiamo la funzione che filtra dai tag già memorizzati
...
function printComments(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var $content = $($template).children('.comment__content'); //rendo la prima lettera maiuscola

    data[i].name = data[i].name.replace(data[i].name[0], data[i].name[0].toUpperCase()); //inserisco dati
...
function getTags(text) {
  ...
  search.ajax = $.ajax({
    ...
    'success': function success(data) {
      ...
      search.tags = JSON.parse(data); //setto pending come false

      search.pending = false; //chiamo funzione che stampa tags
...
function printSuggestions(data) {
  var $wrapper = $('.autocomplete__suggestions'); //se non c'è data svuoto il wrapper

  if (!data) {
    $wrapper.html('');
  } else {
    //se c'è svuoto e stampo i suggerimenti
    $wrapper.html('');
...
__webpack_require__(/*! /home/kja/sites/www/html/php-boolpress/src/js/main.js */"./src/js/main.js");
module.exports = __webpack_require__(/*! /home/kja/sites/www/html/php-boolpress/src/scss/style.scss */"./src/scss/style.scss");
